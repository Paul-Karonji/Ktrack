...
const ADMIN_EMAIL = String(process.env.ADMIN_EMAIL || 'karonjipaul.w@gmail.com').trim();
...
const safeClientUrl = escapeAttribute(CLIENT_URL);
const safeAdminEmail = escapeHtml(ADMIN_EMAIL);
const safeAdminMailto = `mailto:${encodeURIComponent(ADMIN_EMAIL)}`;
                ...
                <table role="presentation" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                    ...
                    <tr>
                        <td style="background: linear-gradient(to right, #4f46e5, #7c3aed); padding: 30px 40px; text-align: center;">
                            <img src="${safeClientUrl}/logo.png" alt="K-Track Logo" style="height: 40px; margin-bottom: 10px; display: inline-block;">
                            <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 700;">${escapeHtml(title)}</h1>
                        </td>
                    </tr>
                    <!-- Content -->
                    <tr>
...
const templates = {
            ...
            <div style="background-color: #eff6ff; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                <table style="width: 100%;">
                    <tr>
                        ...
                        <td style="padding: 4px 0; color: #1f2937; font-weight: 600;">${escapeHtml(user.full_name)}</td>
                    </tr>
                    <tr>
                        <td style="padding: 4px 0; color: #6b7280; font-size: 14px;">Email:</td>
                        <td style="padding: 4px 0; color: #1f2937; font-weight: 600;">${escapeHtml(user.email)}</td>
                    </tr>
                </table>
            </div>
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">Review Application</a>
            </div>
        `;
        return { subject: sanitizeSubject(`👤 New Client: ${user.full_name}`), html: baseTemplate(content, title) };
    },

    newTask: (task, clientName) => {
        const title = 'New Task Submitted';
        const content = `
            <p style="color: #374151; font-size: 16px; margin-bottom: 24px;"><strong>${escapeHtml(clientName)}</strong> has submitted a new task.</p>
            <div style="background-color: #f3f4f6; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                <h3 style="margin: 0 0 5px 0; color: #111827;">${escapeHtml(task.task_name || 'Untitled Task')}</h3>
                <p style="margin: 0 0 10px 0; color: #4b5563; font-style: italic;">${escapeHtml(task.client_name || clientName)}</p>
                <div style="border-top: 1px solid #e5e7eb; padding-top: 10px; margin-top: 10px;">
                    <p style="margin: 0 0 10px 0; color: #374151;">${escapeHtml(task.task_description)}</p>
                    <p style="margin: 0; color: #6b7280; font-size: 14px;">Quantity: ${task.quantity || 1} • Expected: $${task.expected_amount}</p>
                </div>
            </div>
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">View Task</a>
            </div>
        `;
        return { subject: sanitizeSubject(`📋 New Task: ${task.task_name || 'Untitled'} (${clientName})`), html: baseTemplate(content, title) };
    },

    newMessage: (senderName, messageText, taskId, taskName) => {
        const title = 'New Message';
        const content = `
            <p style="color: #374151; font-size: 16px; margin-bottom: 24px;">You have a new message from <strong>${escapeHtml(senderName)}</strong> regarding <strong>${escapeHtml(taskName || 'Task #' + taskId)}</strong>.</p>
            <div style="background-color: #f0fdf4; border-radius: 8px; padding: 20px; margin-bottom: 24px; border-left: 4px solid #22c55e;">
                <p style="margin: 0; color: #1f2937; font-style: italic;">"${escapeHtml(messageText)}"</p>
            </div>
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #22c55e; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">Reply Now</a>
            </div>
        `;
        return { subject: sanitizeSubject(`💬 Message: ${taskName || 'Task #' + taskId}`), html: baseTemplate(content, title) };
    },

    newFileAdmin: (uploaderName, filename, taskId, taskName) => {
        const title = 'New File Uploaded';
        const content = `
            <p style="color: #374151; font-size: 16px; margin-bottom: 24px;"><strong>${escapeHtml(uploaderName)}</strong> uploaded a file to <strong>${escapeHtml(taskName || 'Task #' + taskId)}</strong>.</p>
            <div style="background-color: #eff6ff; border-radius: 8px; padding: 15px; margin-bottom: 24px; display: flex; align-items: center;">
                ...
                <span style="font-weight: 600; color: #1f2937;">${escapeHtml(filename)}</span>
            </div>
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">View File</a>
            </div>
        `;
        return { subject: sanitizeSubject(`📎 New File: ${filename} (Task #${taskId})`), html: baseTemplate(content, title) };
    },
    ...
    taskReceived: (userName, task) => {
        ...
        const content = `
            <p style="color: #374151; font-size: 16px;">Hi ${escapeHtml(userName)},</p>
            ...
            <div style="background-color: #f9fafb; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                <h3 style="margin: 0 0 10px 0; font-size: 16px; color: #111827;">${escapeHtml(task.task_name || 'Untitled Task')}</h3>
                <p style="margin: 0; color: #4b5563;">${escapeHtml(task.task_description)}</p>
            </div>
            <p style="color: #374151; font-size: 16px; margin-bottom: 24px;">We'll get back to you shortly with a quote or any questions.</p>
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">Track Status</a>
            </div>
        `;
        return { subject: sanitizeSubject(`✅ Task Received: ${task.task_name || 'Task #' + task.id}`), html: baseTemplate(content, title) };
    },
    ...
    quoteSent: (userName, task, amount) => {
        ...
        const content = `
            <p style="color: #374151; font-size: 16px;">Hi ${escapeHtml(userName)},</p>
            <p style="color: #374151; font-size: 16px; margin-bottom: 24px;">We've prepared a quote for <strong>${escapeHtml(task.task_name || 'Task #' + task.id)}</strong>.</p>
            ...
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #16a34a; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">Review Quote</a>
            </div>
        `;
        return { subject: sanitizeSubject(`💰 Quote for ${task.task_name || 'Task #' + task.id}`), html: baseTemplate(content, title) };
    },
    ...
    taskStatusUpdate: (userName, task, status) => {
        ...
        const content = `
            ...
            <div style="background-color: #eff6ff; border-radius: 8px; padding: 20px; margin-bottom: 24px; text-align: center;">
                 <p style="margin: 0 0 10px 0; color: #1e3a8a; font-weight: 600;">${escapeHtml(task.task_name || 'Task #' + task.id)}</p>
                 <div style="font-size: 18px; font-weight: 700; color: #2563eb;">${escapeHtml(displayStatus)}</div>
            </div>
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">View Details</a>
            </div>
        `;
        return { subject: sanitizeSubject(`🔄 Update: ${task.task_name || 'Task #' + task.id} is ${displayStatus}`), html: baseTemplate(content, title) };
    },
    ...
    newFileClient: (userName, filename, taskId, taskName) => {
        ...
        const content = `
            <p style="color: #374151; font-size: 16px;">Hi ${escapeHtml(userName)},</p>
            <p style="color: #374151; font-size: 16px; margin-bottom: 24px;">We've just uploaded a new file for <strong>${escapeHtml(taskName || 'Task #' + taskId)}</strong>.</p>
            ...
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #db2777; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">Download File</a>
            </div>
        `;
        return { subject: sanitizeSubject(`📦 New File: ${filename}`), html: baseTemplate(content, title) };
    },
    ...
    newMessageClient: (userName, messageText, taskId, taskName) => {
        ...
        const content = `
            <p style="color: #374151; font-size: 16px;">Hi ${escapeHtml(userName)},</p>
            <p style="color: #374151; font-size: 16px; margin-bottom: 24px;">You have a new message regarding <strong>${escapeHtml(taskName || 'Task #' + taskId)}</strong>.</p>
            <div style="background-color: #f3f4f6; border-radius: 8px; padding: 20px; margin-bottom: 24px; border-left: 4px solid #4f46e5;">
                <p style="margin: 0; color: #1f2937; font-style: italic;">"${escapeHtml(messageText)}"</p>
            </div>
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">View Conversation</a>
            </div>
        `;
        return { subject: sanitizeSubject(`\u{1F4AC} New Message: ${taskName || 'Task #' + taskId}`), html: baseTemplate(content, title) };
    },
    ...
    registrationConfirmation: (user) => {
            ...
            <div style="background-color: #eff6ff; border-radius: 8px; padding: 16px; margin-bottom: 24px;">
                <table style="width: 100%;">
                    <tr>
                        ...
                        <td style="padding: 3px 0; color: #1f2937; font-weight: 600; font-size: 13px;">${escapeHtml(user.full_name)}</td>
                    </tr>
                    <tr>
                        <td style="padding: 3px 0; color: #6b7280; font-size: 13px;">Email:</td>
                        <td style="padding: 3px 0; color: #1f2937; font-weight: 600; font-size: 13px;">${escapeHtml(user.email)}</td>
                    </tr>
                    <tr>
        ...
        `;
        return { subject: sanitizeSubject(`\u2705 Registration Received \u2014 K-Track`), html: baseTemplate(content, title) };
    },
    ...
    taskAssigned: (userName, task) => {
        ...
        const content = `
            <p style="color: #374151; font-size: 16px;">Hi <strong>${escapeHtml(userName)}</strong>,</p>
            <p style="color: #374151; font-size: 16px; margin-bottom: 24px;">
            ...
            <div style="background-color: #f9fafb; border-radius: 8px; padding: 20px; margin-bottom: 24px; border-left: 4px solid #4f46e5;">
                <h3 style="margin: 0 0 8px 0; color: #111827; font-size: 16px;">${escapeHtml(task.task_name || 'New Task')}</h3>
                <p style="margin: 0 0 12px 0; color: #4b5563; font-size: 14px; line-height: 1.6;">${escapeHtml(task.task_description || '')}</p>
            ...
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">View My Dashboard</a>
            </div>
        `;
        return { subject: sanitizeSubject(`📋 New Task Added: ${task.task_name || 'New Task'}`), html: baseTemplate(content, title) };
    },
    ...
    paymentReminderSummary: (userName, tasks, totalDue) => {
        ...
        const rows = tasks.map((task) => `
            <tr>
                <td style="padding: 10px 0; border-bottom: 1px solid #e5e7eb;">
                    <div style="font-weight: 700; color: #111827;">${escapeHtml(task.task_name || `Task #${task.id}`)}</div>
                    <div style="font-size: 12px; color: #6b7280;">${escapeHtml(task.payment_state_label)}</div>
                </td>
        ...
        const content = `
            ...
            <div style="text-align: center;">
                <a href="${safeClientUrl}/dashboard" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">Open My Dashboard</a>
            </div>
        `;

        return {
            subject: sanitizeSubject(`K-Track Payment Reminder: $${Number(totalDue || 0).toFixed(2)} Due`),
    ...
    accountRejected: (userName) => {
        ...
        const content = `
            ...
            <p style="color: #374151; font-size: 14px; margin-bottom: 24px;">
                You can contact us at <a href="${safeAdminMailto}" style="color: #4f46e5;">${safeAdminEmail}</a> to discuss further.
            </p>
        `;
        return { subject: sanitizeSubject(`K-Track Registration Update`), html: baseTemplate(content, title) };
    },
    ...
    accountSuspended: (userName) => {
        ...
        const content = `
            ...
            <p style="color: #374151; font-size: 14px; margin-bottom: 0;">
                Reach us at <a href="${safeAdminMailto}" style="color: #4f46e5;">${safeAdminEmail}</a>.
            </p>
        `;
    ...
    accountReactivated: (userName) => {
        ...
        const content = `
            ...
            <div style="text-align: center;">
                <a href="${safeClientUrl}/login" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600;">Log In Now</a>
            </div>
        `;
        return { subject: sanitizeSubject(`✅ K-Track Account Reactivated`), html: baseTemplate(content, title) };
    }
};
